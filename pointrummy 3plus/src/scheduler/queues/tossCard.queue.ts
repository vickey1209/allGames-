...
import url from "url"
import { tossCardStartProcess } from "../processes/tossCardStart.process";
...
if (NODE_ENV === "PRODUCTION") {
  ...
  tossCardStartQueue = new Bull('tossCardStart_3Plus', { redis : { host : hostname, port:port, db : Number(REDIS_DB) } });
}else{
  tossCardStartQueue = new Bull(`tossCardStart_3Plus`, {
    redis: SchedulerRedisConfig,
  });
}

export async function tossCardStart(data: any){
...
tossCardStartQueue.process(tossCardStartProcess);

// export = tossCardStart;